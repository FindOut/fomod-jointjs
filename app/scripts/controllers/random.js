  ...
  .controller('RandomCtrl', function($scope, $timeout, fbref, attrMap) {
    ...
    function addRandomContent(model) {
      ...
      for (var i = 0; i < parameters.objectCount * parameters.objectCount && relations.length < parameters.relationCount; i++) {
        var fromi = _.random(parameters.objectCount - 1),
          toi = _.random(parameters.objectCount - 1);
        var relKey = fromi + '_' + toi,
          relKeyRev = toi + '_' + fromi;
        console.log('relKey=' + relKey);
        if ((parameters.allowMultipleRelsBetweenObjectPair || !(relMap[relKey] || relMap[relKeyRev])) && (parameters.allowRelationToSelf || fromi != toi)) {
          var rel = {
            id: 'relation' + relations.length,
            from: objects[fromi].id,
            to: objects[toi].id
          };
          relMap[relKey] = rel;
          relations.push(rel);
    ...
    $scope.create = function() {
      ...
      $timeout(function() {
        window.location.href = '#/models/' + fbModelRef.key()
      });
    }
  });