...
angular.module('fomodApp')
  ...
  .service('CreateRelationCommand', function(data, FomodRelation) {
    return function(id, from, to) {
      var relation = new FomodRelation({
        id: id,
        from: from,
        to: to
      });
  ...
  .service('ChangeDataAttributeCommand', function(data) {
    return function(nameValueMap) {
      var oldNameValueMap;
      this.do = function() {
        oldNameValueMap = _.reduce(nameValueMap, function(result, value, key) {
  ...
  .service('AutoLayoutCommand', function(autoLayouter) {
    return function(graph) {
      ...
      this.undo = function() {
        _.each(oldPositions, function(posItem) {
          graph.getCell(posItem.cellId).set('position', posItem.position)
        });
      };
      this.redo = function() {
        this.do();
  ...
  .service('commander', function() {
    ...
    var fireCommandDone = function(cmd, what) {
      ...
      _.each(commandListeners, function(commandListener) {
        commandListener(cmd, what);
      });
    };
